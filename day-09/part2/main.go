...
func getExtrapolates(histories [][]int) []int {
	...
	for h, history := range histories {
		...
		// Now extrapolating the first value
		for i := len(relations) - 1; i >= 1; i-- {
			lastCellIndex := len(relations[i]) - 1
			// Actually adding the values at the end of the array still to avoid weird stuff.
			// So, it's a mix of using the first array cell of the previous line and the actual last 
			// 	for the current one.
			relations[i-1] = append(relations[i-1], relations[i-1][0]-relations[i][lastCellIndex])
		}
